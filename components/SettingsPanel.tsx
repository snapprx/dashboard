    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4"
          ...
          <div className={`w-full max-w-lg rounded-2xl border shadow-2xl p-6 space-y-6 max-h-[90vh] overflow-y-auto ${bg}`}>
            ...
            {/* Météo */}
            <div>
              <p className="text-sm font-semibold mb-2">📍 Localisation météo</p>
              ...
              {customCity && (
                <div className="grid grid-cols-3 gap-2">
                  <input value={draft.weather.city} placeholder="Ville"
                    onChange={e => setDraft(s => ({ ...s, weather: { ...s.weather, city: e.target.value } }))}
                    className={`rounded-lg border px-2 py-1.5 text-sm outline-none ${inp}`} />
                  <input value={draft.weather.lat} placeholder="Latitude"
                    onChange={e => setDraft(s => ({ ...s, weather: { ...s.weather, lat: e.target.value } }))}
                    className={`rounded-lg border px-2 py-1.5 text-sm outline-none ${inp}`} />
                  <input value={draft.weather.lon} placeholder="Longitude"
                    onChange={e => setDraft(s => ({ ...s, weather: { ...s.weather, lon: e.target.value } }))}
                    className={`rounded-lg border px-2 py-1.5 text-sm outline-none ${inp}`} />
                </div>
              )}
            </div>
              ...
              {adding && (
                <div className={`rounded-xl border p-4 space-y-3 ${rowBg}`}>
                  ...
                  <div className={`text-xs space-y-0.5 ${sub}`}>
                    <p>• <strong>iCloud</strong> : Calendrier → agenda → ⋯ → Copier le lien iCal</p>
                    <p>• <strong>Google</strong> : Paramètres → agenda → Intégration → iCal</p>
                    <p>• Les liens <code className="bg-black/10 px-1 rounded">webcal://</code> sont convertis automatiquement</p>
                  </div>
                  <div>
